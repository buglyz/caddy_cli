...
sanitize_name() {
    echo "$1" | tr '/: ,' '____' | tr -cd 'A-Za-z0-9._-'
}
...
validate_config_file() {
    ...
    if cloudflare_env_configured; then
        caddy validate --envfile "$CLOUDFLARE_ENV_FILE" --config "$config_path" --adapter caddyfile >"$LAST_VALIDATE_LOG" 2>&1
    else
        caddy validate --config "$config_path" --adapter caddyfile >"$LAST_VALIDATE_LOG" 2>&1
    fi
}
...
cmd_list() {
    ...
    if cloudflare_env_configured; then
        echo "已启用（/etc/caddy/cloudflare.env）"
    else
        echo "未启用"
    fi
    echo
...
cmd_doctor() {
    ...
    if command_exists caddy; then
        ...
        if caddy list-modules 2>/dev/null | grep -q '^dns.providers.cloudflare$'; then
            ...
            echo "[WARN] 未检测到 Cloudflare DNS 模块"
        fi
    else
        echo "[NO] 未安装 caddy"
    fi
...
cmd_update() {
    ...
    target_bin="/usr/local/bin/caddyctl"
    if [[ -z "$url" ]]; then
        fail "此版本未内置自动更新地址，请手动更新脚本，或临时设置 CADDYCTL_UPDATE_URL 后再执行 update。"
        return 1
    fi
    target_alias="/usr/local/bin/c"
    tmp="$(mktemp)"
...
用法:
  ...
  c list
  c email [邮箱]
  c cloudflare [token|clear|status]
  c timeout [秒|default]
...
main() {
    ...
    case "$cmd" in
        ...
        cloudflare) shift; require_command caddy; with_global_lock run_mutation cloudflare cmd_cloudflare "${1:-}" ;;
